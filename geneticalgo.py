...
class geneticalgo(object):
    ...
    def select(self):
        ...
        # 选取后进行染色体交换后
        np_pop_score_list = np.array(self.pop_score_list, dtype=np.float64)
        ...
        to_cross = []
        temp_off_elitism = []
        elitism = np_pop_score_list[:, 0][0:(self.elitism_rate*self.population_size)]
        # 50个个体中0.1%的即前五的保留,然后从全集里提取45*2个组成45对进行交叉,(前五+交叉后的45个)*变异概率=新种群
        for item in range((self.population_size * (1-self.elitism_rate)) * 2):
            rand = random.random()
            for index,value in enumerate(self.pop_score_list):
                if index == 0:
                    if 0 <= rand <= value[1]:
                        to_cross.append(value[0])
                        break
                else:
                    if self.pop_score_list[index-1][1] <= rand <= self.pop_score_list[index][1]:
                        to_cross.append(value[0])
                        break
        ...
        assert len(self.poplist) == len(self.population_size)
